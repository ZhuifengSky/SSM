...
<%@ attribute name="isHideHead" type="java.lang.Boolean" required="false" description="消息内容"%>
<%@ attribute name="type" type="java.lang.String" description="消息类型：info、success、warning、error、loading"%>
<script src="../static/js/jquery-3.2.1.min.js" type="text/javascript"language="javascript"></script>
...
<c:if test="${not empty content}">
	...
	<c:if test="${!isHideHead }">
	<div id="messageBox" class="alert alert-${ctype} <c:if test="${empty content}">hide</c:if>"><button data-dismiss="alert" class="close">×</button>${content}</div> 
	</c:if>
